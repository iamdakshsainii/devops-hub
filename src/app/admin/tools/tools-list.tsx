...
export function ToolsList({ initialData }: { initialData: any[] }) {
  ...
  const handleToggleStatus = async (id: string, currentStatus: string) => {
    ...
    try {
      ...
      if (res.ok) {
        setLocalData(localData.map((t) => t.id === id ? { ...t, status: newStatus } : t));
      }
    } catch { }
    setLoadingId(null);
  };
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Are you sure you want to soft delete this tool?")) return;
    setLoadingId(id);
    try {
      const res = await fetch(`/api/admin/tools/${id}`, { method: "DELETE" });
      if (res.ok) {
          setLocalData(localData.filter((t) => t.id !== id));
      }
      ...
      <div className="flex flex-col sm:flex-row justify-between gap-3 items-center">
        ...
        <Link href="/admin/tools/new">
          <Button size="sm" className="gap-2 h-9">
            <PlusCircle className="h-4 w-4" /> New Tool
          </Button>
        </Link>
      </div>
          ...
          filtered.map((item) => (
            <div key={item.id} className="grid grid-cols-1 md:grid-cols-6 gap-4 p-4 border-b border-border/10 items-center text-sm last:border-0 hover:bg-muted/5 transition-colors">
              ...
              <div className="hidden md:block">
                <Badge variant="secondary" className="font-semibold text-xs">{item.category}</Badge>
              </div>
              <div className="hidden md:block">
                <Badge variant="outline" className="text-xs font-medium">{item.difficulty}</Badge>
              </div>
