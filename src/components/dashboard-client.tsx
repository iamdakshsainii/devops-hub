...
import { Terminal, Award, Zap, Flame, Trophy, CheckCircle2, Map, ArrowRight, Shield, Clock, BookOpen } from "lucide-react";
...
export function DashboardClient({ user, currentRoadmap, allStats, progress }: any) {
  ...
  const getIntensityColor = (count: number) => {
     ...
     if (count <= 2) return "bg-primary/20 border-primary/10 text-primary";
     if (count <= 5) return "bg-primary/40 border-primary/20 text-white";
     if (count <= 10) return "bg-primary/70 border-primary/30 text-white";
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
         {[
           ...
           { label: "Points Earned", count: allStats.points, icon: <Zap className="h-4 w-4" /> },
            ...
            <Card key={stat.label} className="bg-card/50 backdrop-blur border overflow-hidden">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
         <div className="lg:col-span-2 space-y-8">
            ...
            <Card className="rounded-2xl border bg-card/50 backdrop-blur shadow-sm overflow-hidden">
                 {/* Header to Toggle view wrapper */}
                 <div className="p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b border-border/40">
                     <div>
                         <h3 className="font-bold text-foreground text-base">Activity Heatmap</h3>
                         <p className="text-[11px] text-muted-foreground mt-0.5">
                              {view === 'week' ? 'Past 7 days completions' : view === 'month' ? 'Current month overview' : 'Past year of learning activity'}
                         </p>
                     </div>
                     <div className="flex bg-muted/30 p-1 rounded-xl border">
                         {[
                              ...
                              <button
                                   ...
                                   className={`text-xs px-2.5 py-1.5 rounded-lg font-bold transition-all ${view === tab.value ? 'bg-background text-primary shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}
                              >
                                   {tab.label}
                              </button>
                         ))}
                 ...
                 <div className="p-5 space-y-4">
                                 ...
                                 {monthActivity.map((cell, index) => {
                                      ...
                                      return (
                                           ...
                                           >
                                                {cell.date && (
                                                     <>
                                                          <span className="text-[9px] font-bold text-background/80 drop-shadow-sm">{cell.date.getDate()}</span>
                     ...
                     <div className="pt-2 flex justify-between items-center text-[10px] text-muted-foreground font-semibold">
                          <Link href="/dashboard/achievements" className="flex-1">
                               <Button variant="outline" className="w-full text-xs rounded-xl flex items-center justify-center gap-1 group bg-background shadow-xs hover:bg-muted/30">
